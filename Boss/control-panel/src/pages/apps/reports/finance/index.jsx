...
// import AddProType from '../../../../views/apps/property/type/addProTypeModel'
import TableHeaderProType from 'src/views/apps/property/type/TableHeader-proType'
...
const renderClient = row => {
  if (row.avatar.length) {
    ...
    return (
      ...
      >
        {getInitials(row.fullName ? row.fullName : 'Iker Paster')}
      </CustomAvatar>
    )
  }
}
...
const UserList = ({ apiData }) => {
  ...
  const [modalOpenHost, setModalOpenHost] = useState(false);
  const openModalHost = () => {setModalOpenHost(true);setIsHost(true);}
  const closeModalHost = () => {setModalOpenHost(false); setIsHost(false);}
...
<div className='bg-red-400x my-5 py-3'>

<PageHeader
  title={<Typography variant='h5'>Financial Overview</Typography>}
  subtitle={
    <Typography variant='body2'>
      Explore a concise summary of financial data across all property types.
    </Typography>
  }
/>
        ...
        <Card>
          ...
          <Divider />
          <TableHeaderProType value={value} handleFilter={handleFilter} toggle={openModalAdmin} />
          <DataGrid
            autoHeight
            rows={store.data}
            columns={columns}
