...
import { BorderColorOutlined, DeleteOutlineOutlined, Diversity1, EditNoteOutlined, PeopleOutlined, Person, Remove } from '@mui/icons-material'
import useSWR from 'swr'
import AddAmenityCategory from '../amenities/ele/AddAmenityCategory'
import DialogDeleteAmenityCategory from '../amenities/ele/DialogDeleteAmentyCategory'
...
const ViewBEDType= () => {
  ...
  // get all positions related to the department 
    let url = `${process.env.NEXT_PUBLIC_WEB_URL}/admin/bed-type/all/`;
  ...
  return (
    <div>
{/*         
        <div onClick={() => setShow(true)} className='py-2  bg-violet-400 px-3 rounded-lg text-white cursor-pointer'>
       Bed Type
        </div> */}
      ...
      >
        ...
        >
          ...
          <Box sx={{ mb: 4, textAlign: 'center' }} className="flex justify-between items-center">
           ...
           <Typography  className='bg-violet-300  p-2 rounded-lg cursor-pointer'><span className='text-white' onClick={()=>setAddPo((prev)=>!prev)}>New Bed Type </span></Typography>
...
<div className={` bg-green-400x ${showAddPos ? '-mt-10': '-mt-1'} absolutex h-[calc(100vh-29rem)] menu pr-10 mt-5`} onClick={()=>setAddPo(prev=>!prev)}  >
            ...
            {data?.map((member,index) => {
              return (
                ...
                >
                  ...
                  <ListItemText
                    primary={member.name}
                    secondary={`${member?.amenities?.length && member?.amenities?.length ? member?.amenities?.length : '0' } ${amenity_Text}`}
                  ...
                  <ListItemSecondaryAction sx={{ right: 0 }}>
                      ...
                      <div className="relativex flex items-center gap-2">
                        ...
                        <DialogDeleteAmenityCategory id={member._id}/> 
                        </span>
